...
> Email podcast ideas → agents pick the best → research → voice (ElevenLabs) → publish to a real RSS feed → reply with playable links.

**onloop** is a fork of [`vercel-labs/open-agents`](https://github.com/vercel-labs/open-agents) (the OSS reference for durable agents on Vercel), adapted for an email-in podcast pipeline built during the OpenCode Buildathon.

- Live execution plan: [`SPEC.md`](./SPEC.md)
- Track: MaaS (primary) · Bonus: Revenue + Virality
- Stack overlay on top of open-agents: `inboundemail`, ElevenLabs (TTS + SFX), Langfuse, `@xyflow/react`, `podcast` npm pkg.

The open-agents docs below describe the durable-agent substrate onloop rides on.

---

# Open Agents (upstream)
